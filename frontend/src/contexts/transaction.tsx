...
export const createRaffle = async (
  ...
  try {
    ...
    let ownerNftAccount = await getAssociatedTokenAccount(
      userAddress,
      nft_mint,
    );
...
export const claimReward = async (
  ...
  const raffleKey = await getRaffleKey(nft_mint);
  if (raffleKey === null) return;
  const srcNftTokenAccount = await getAssociatedTokenAccount(
    globalAuthority,
    nft_mint,
  );
  ...
  try {
    const srcPreyTokenAccount = await getAssociatedTokenAccount(
      globalAuthority,
      PREY_TOKEN_MINT,
    );
    const claimerPreyTokenAccount = await getAssociatedTokenAccount(
      userAddress,
      PREY_TOKEN_MINT,
    );
